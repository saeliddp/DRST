...
class HMM():
  ...
  def _smooth_convert_count_to_prob(self, key, count_dict):
    sum_count = np.sum([count_dict[key][k] for k in count_dict[key]])
    sum_count += len(self.label_to_ix) - len(count_dict[key])
    sum_count = np.log(sum_count)
    for k in count_dict[key]:
      count_dict[key][k] = np.log(count_dict[key][k]) - sum_count
    count_dict[key][OTHER] = np.log(1) - sum_count
  ...
  def train(self, tr_src_fpath, tr_target_fpath, generator_fn, smoothing=1, last_k_to_unknown=100):
    ...
    for tokens, labels in generator_fn(tr_src_fpath, tr_target_fpath):
      ...
      for t, l in zip(tokens, labels):
        ...
        if l not in emission_counts[tok]:
          emission_counts[tok][l] = 0
        
        emission_counts[tok][l] += 1
        prev_label = l

    for token in emission_counts:
      self._smooth_convert_count_to_prob(token, emission_counts)
    self.emission_probs = emission_counts 

    for label in transition_counts:
      self._smooth_convert_count_to_prob(label, transition_counts)
  ...
  def _viterbi_step(self, prev_scores, prev_labels, curr_token):
    ...
    for label in self.emission_probs[curr_token]:
      ...
      for i, prev_val in enumerate(prev_scores):
        ...
        if curr_token in self.emission_probs:
          val += self.emission_probs[curr_token][label]
        else:
          if label in self.emission_probs[UNKNOWN]:
            val += self.emission_probs[UNKNOWN][label]
          else:
            val += self.emission_probs[UNKNOWN][OTHER]
